...
server_connection_string = f"Driver={{ODBC Driver 18 for SQL Server}};Server={sql_db_server},1433;Uid=aiavatar;Pwd={sql_db_password};Encrypt=yes;TrustServerCertificate=no;Connection Timeout=30;"
database_connection_string = server_connection_string + f"Database={sql_db_name};"
    ...
    order_details = [
        {
            ...
            "expected_delivery_date": (
                today + timedelta(days=order.days_to_delivery)
            ).strftime("%Y-%m-%d"),
        }
        for order in orders
    ]
...
def order_product(account_id, product_name, quantity=1):
    ...
    if place_orders:
        execute_sql_query(query, params=params)
...
def display_product_info(product_info, display_size=40):
    ...
    image_url = (
        blob_sas_url.split("?")[0] + f"/{image_file}?" + blob_sas_url.split("?")[1]
    )
...
def get_product_information(user_question, categories="*", top_k=1):
    ...
    data = {
        "vectors": [
            {"value": vector, "fields": "description_vector", "k": top_k},
        ],
